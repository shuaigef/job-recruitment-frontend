            ...
            >
                <div v-for="systemNotice in systemNoticeDataList" :key="systemNotice.id">
                    <a-card>
                        <a-card-meta>
                            <template #title>
                                <span>{{ systemNotice.informationTitle }}</span>
                                <span style="margin-left: 20px">{{ systemNotice.createTime}}</span>
                            </template>
                            <template #description>
                                {{ systemNotice.informationContent }}
                            </template>
                        </a-card-meta>
                    </a-card
                    >
...
onMounted(async () => {
    ...
    await getUserChatListRequest(companyInfo.value.id)
        .then(res => {
...
const send = () => {
    ...
    if (!text.value) {
        ...
        if (typeof (WebSocket) == "undefined") {
            ...
            remoteUserMessageHistory.value.push({fromName: companyInfo.value.companyName, fromAvatar: companyInfo.value.companyAvatar, content: text.value})
